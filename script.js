...
const selected=[
 ...
 ["شاعران وارث آب و خرد روشنی‌اند",1]
];
...
function cursor(){
  cx+=(mx-cx)*.2;
  cy+=(my-cy)*.2;
  const cEl=$('#cursorLight');
  if(cEl) cEl.style.transform=`translate(${cx}px,${cy}px) translate(-50%,-50%)`;
...
let lamp=0;const lampEl=document.createElement('button');lampEl.className='floatingLamp';lampEl.setAttribute('aria-label','چراغ');lampEl.innerHTML='✦';document.body.appendChild(lampEl);lampEl.onclick=()=>{lamp=1-lamp;document.body.classList.toggle('bright',!!lamp)};
